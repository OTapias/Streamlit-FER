...
# Image Page
    ...
    st.markdown("**Rostros Detectadas**")
    kpil_text = st.markdown('0')

    max_faces = st.sidebar.number_input('Número Máximo de Rostros', value=1, min_value=1)
    st.sidebar.markdown('---')

    detection_confidence = st.sidebar.slider('Mín. Factor de Confianza', min_value=0.0,max_value=1.0,value=0.5)
    st.sidebar.markdown('---')

    ## Output
    st.markdown('## Resultado')
    img_file_buffer = st.sidebar.file_uploader("Cargar Una Imagen", type=["jpg","jpeg","png"])
